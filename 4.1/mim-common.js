...
!function () {
    ...
    /*成功或失敗的提示框*/
    jQuery.fn.prompt = MIM.prompt = function (state, type, time) {
        /*
        * @param time消失时间（毫秒级），type（true成功或false失败）state提示的语句，
        * */
        var set = "";
        clearTimeout(set);
        var div;
        switch (type) {
            case "success":
                var div = $("<div class='mim-prompt_success_box'></div>").animate({
                    top: "50px",
                    opacity: '1'
                }, 500).appendTo($("body"));
                $("<p class='mim-prompt_success_p'><i class='iconfont'>&#xe63b;</i><span>" + state + "</span></p>").appendTo(div);
                break;
            case "error":
                var div = $("<div class='mim-prompt_error_box'></div>").animate({
                    top: "50px",
                    opacity: '1'
                }, 500).appendTo($("body"));
                $("<p class='mim-prompt_error_p'><i class='iconfont'>&#xe635;</i><span>" + state + "</span></p>").appendTo(div);
                break;
            case "warning":
                var div = $("<div class='mim-prompt_warning_box'></div>").animate({
                    top: "50px",
                    opacity: '1'
                }, 500).appendTo($("body"));
                $("<p class='mim-prompt_warning_p'><i class='iconfont'>&#xe61b;</i><span>" + state + "</span></p>").appendTo(div);
                break;
            case "info":
                var div = $("<div class='mim-prompt_info_box'></div>").animate({
                    top: "50px",
                    opacity: '1'
                }, 500).appendTo($("body"));
                $("<p class='mim-prompt_info_p'><i class='iconfont'>&#xe613;</i><span>" + state + "</span></p>").appendTo(div);
                break;
        }
